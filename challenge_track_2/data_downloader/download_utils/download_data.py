...
def laser_scanner_point_clouds_for_visit_id(visit_id):
    ...
    point_cloud_to_visit_id_mapping_filepath = os.path.join("data_splits", point_cloud_to_visit_id_mapping_filename)
...
def download_laser_scanner_point_clouds(laser_scanner_point_cloud_id, visit_id, download_dir):
    ...
    laser_scanner_point_clouds_folder_path = os.path.join(download_dir, str(visit_id))
...
def download_assets_for_visit_id(visit_id, download_dir, split, dataset_assets, metadata):
    url_prefix = ""
    file_names = []
...
def download_assets_for_video_id(visit_id, video_id, download_dir, split, dataset_assets, metadata):
    url_prefix = ""
    file_names = []
    if not dataset_assets:
        ...
        file_names = video_raw_files(video_id, dataset_assets, metadata)
    ...
    for i, file_name in enumerate(file_names):
        ...
        if file_name.endswith('.zip') and os.path.isfile(dst_path):
            unzip_file(file_name, dst_dir, keep_zip=False)

    if 'vid_mp4' in  dataset_assets:
        convert_mov_to_mp4(video_id, download_dir, metadata)
...
# def download_laser_scan(video_id, split, download_dir):
#     keep_zip = False

#     # args = parser.parse_args()
#     assert video_id is not None, \
#         'video_id must be specified'
#     assert video_id is None or split is not None, \
#         'given video_id the split argument must be specified'

#     if video_id is not None:
#         video_ids_ = video_id
#         splits_ = splits = [split, ] * len(video_ids_)
